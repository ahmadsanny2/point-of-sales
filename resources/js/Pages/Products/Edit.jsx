        ...
        <AuthenticatedLayout header="Edit Produk">
            ...
            <div className="max-w-3xl mx-auto bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden transition-colors">
                <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center bg-slate-50 dark:bg-slate-900/50 transition-colors">
                    ...
                    <h3 className="text-lg font-medium text-slate-800 dark:text-slate-100">Ubah Data Produk</h3>
                </div>
                ...
                <form onSubmit={submit} className="p-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div className="md:col-span-2">
                            ...
                            <input id="name" type="text" className={inputClass(errors.name)} value={data.name} onChange={(e) => setData('name', e.target.value)} />
                        ...
                        <div>
                            ...
                            <input id="sku" type="text" className={inputClass(errors.sku)} value={data.sku} onChange={(e) => setData('sku', e.target.value)} />
                        ...
                        <div>
                            ...
                            <input id="price" type="number" min="0" className={inputClass(errors.price)} value={data.price} onChange={(e) => setData('price', e.target.value)} />
                            {errors.price && <p className="mt-1 text-sm text-red-600 dark:text-red-400">{errors.price}</p>}
                        </div>

                        {/* Stock */}
                        <div>
                            <label htmlFor="stock" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Stok Tersedia</label>
                            <input id="stock" type="number" min="0" className={inputClass(errors.stock)} value={data.stock} onChange={(e) => setData('stock', e.target.value)} />
                        ...
                        <div className="md:col-span-2">
                            ...
                            {product.image_path && (
                                <div className="mb-2">
                                    <img src={`/storage/${product.image_path}`} alt={product.name} className="h-20 w-20 object-cover rounded shadow-sm border border-slate-200 dark:border-slate-600" />
                                </div>
                            )}
                            ...
                            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Biarkan kosong jika tidak ingin mengubah gambar.</p>
